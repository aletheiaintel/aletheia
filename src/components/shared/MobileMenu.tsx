	...
	return (
		<div className="md:hidden">
			<Sheet open={isOpen} onOpenChange={setIsOpen}>
				<SheetTrigger asChild>
					<button
						className="flex h-9 w-9 items-center justify-center rounded-lg bg-transparent transition-colors hover:bg-[#f0f1f3]"
						aria-label="Open menu"
					>
						<Menu className="h-4 w-4 text-[#0F1A2E]" />
					</button>
				</SheetTrigger>

				<SheetContent
					side="right"
					className="w-[75%] max-w-[320px] border-l border-black/6 bg-[rgba(245,246,248,0.98)] p-0 backdrop-blur-xl"
					showCloseButton={false}
				>
					<div className="flex items-center justify-between border-b border-black/6 px-5 py-5">
						<SheetTitle asChild>
							...
							>
								<BrandLogo />
								<span className="text-[16px] font-semibold tracking-[-0.02em] text-[#0F1A2E]">
									Aletheia
								</span>
							</Link>
						</SheetTitle>
						...
						<button
							onClick={() => setIsOpen(false)}
							className="flex h-8 w-8 items-center justify-center rounded-lg border border-black/8 bg-white transition-colors hover:bg-[#f0f1f3]"
							aria-label="Close menu"
						>
						...
						{headerLinks.map(({ href, label }) => (
							<Link
								...
								className={cn(
									"group relative flex items-center rounded-lg px-4 py-3 text-[15px] font-medium transition-all duration-200",
									isActive(href)
										? "bg-[#E8F5EE] text-[#C9981A] ring-1 ring-[#C9981A]/20"
										: "text-[#5A6478] hover:bg-black/4 hover:text-[#0F1A2E]",
								)}
							>
								{isActive(href) && (
									<>
					...
					<div className="px-4 pt-2">
						<Button
							asChild
							className="h-12 w-full rounded-[8px] bg-[#0A1628] px-6 text-[15px] font-medium shadow-none transition-all hover:-translate-y-0.5 hover:bg-[#162340]"
						>
							<Link
								href="#contact"
								onClick={() => setIsOpen(false)}
					...
					<div className="absolute bottom-0 left-0 right-0 border-t border-black/6 p-4">
