...
// 					ease: "easeInOut",
// 				}}
// 				className="absolute -top-24 right-[8%] w-[420px] h-[420px] rounded-full bg-[radial-gradient(circle,#F2C594_0%,#E8B5D4_50%,transparent_70%)] opacity-50"
// 			/>
...
// 					delay: 1,
// 				}}
// 				className="absolute top-[30%] -left-24 w-[340px] h-[340px] rounded-full bg-[radial-gradient(circle,#D4E8C2_0%,#E8D5B7_45%,transparent_70%)] opacity-45"
// 			/>
...
// 					delay: 3,
// 				}}
// 				className="absolute bottom-[15%] right-[2%] w-[280px] h-[280px] rounded-full bg-[radial-gradient(circle,#BAE6FD_0%,#7DD3FC_25%,transparent_70%)] opacity-25"
// 			/>
...
// 					delay: 5,
// 				}}
// 				className="absolute -bottom-20 left-[20%] w-[380px] h-[380px] rounded-full bg-[radial-gradient(circle,#E0F2FE_0%,#BAE6FD_30%,transparent_70%)] opacity-35"
// 			/>
...
// 					delay: 2,
// 				}}
// 				className="absolute top-[60%] left-[45%] w-[200px] h-[200px] rounded-full bg-[radial-gradient(circle,#FFF8F0_0%,transparent_70%)] opacity-60"
// 			/>
...
// 							<div
// 								key={svc.index}
// 								className="svc-card opacity-0 p-7 md:p-10 bg-[#FDFAF5] border border-black/[0.07] rounded-[20px] shadow-[0_2px_16px_rgba(0,0,0,0.05)] transition-[border-color] duration-350 ease-[ease] cursor-default hover:border-black/[0.14]"
// 							>
// 								<div className="grid grid-cols-1 md:grid-cols-[88px_1fr_190px] gap-6 md:gap-10 items-start">
...
// 									<div className="card-right flex flex-row md:flex-col items-start md:items-end justify-between md:justify-start gap-4 md:gap-6 opacity-0">
...
export default function Services() {
	...
	useEffect(() => {
		...
		const ctx = gsap.context(() => {
			...
			// Stack all cards: card 0 visible, rest off-screen to the right
			...
			// Animate card-inner elements on the first card immediately
			...
			// Track which card inner elements have been animated in
			...
			ScrollTrigger.create({
				...
				onUpdate: (self) => {
					...
					// Animate card inner elements when a new card becomes dominant
					...
					cards.forEach((card, i) => {
						if (i < activeIndex) {
							// Already passed — pinned behind, fully visible but lower z
							gsap.set(card, {
								x: "0%",
								opacity: 1,
								scale: 1,
							...
							// Current card: stays fully visible, slides slightly left only at the very end
							// No opacity change — always fully readable
							gsap.set(card, {
								x: `${-cardProgress * 5}%`,
								opacity: 1,
							...
							// Incoming card: slides in from right, always fully opaque once it enters
							// Start opacity at 1 immediately so it's never invisible mid-slide
							gsap.set(card, {
								x: `${(1 - cardProgress) * 100}%`,
								opacity: 1,
								scale: 1,
								zIndex: cards.length - i + 1, // sits on top of current
							});
						} else {
							// Future cards: off-screen right, hidden
							gsap.set(card, {
								x: "100%",
								opacity: 1,
								scale: 1,
